...
scrape.scrapeScraperJSON =
  ...
  for (var key in definition) {
    try {
      ...
      var patternProperties = element.patternProperties;
      ...
      for (var i = 0; i < matches.length; i++) {
        ...
        if (res) {
          ...
          // check if there's a regex to extract a sub string
          if (patternProperties)
          {
             var pattern = new RegExp(patternProperties, "g");
             var matchArr = pattern.exec(res);

             if(matchArr && matchArr.length > 0) {
                 res = matchArr[1];
             }
             else {
                log.debug('Regex did not match: ', pattern.toString());
             }
          }
